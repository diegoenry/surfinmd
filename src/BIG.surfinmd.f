! usar unit=1*
        ...
        write(*,*) "This version will not compute std-deviation"
        write(*,*) "Continue ? (Y/N)"
        read(*,"(a1)") warning
        if(warning.eq."y".or.warning.eq."Y") then
        continue
        else
        STOP
        endif
...
!!! removido v1.05r4           do j=1,maxamn
!!! removido v1.05r4            desvio(nframes,j)=amnarea(j)
!!! removido v1.05r4            enddo
...
101     continue
        ...
        do i=1,namn
!!! removido v1.05r4         dp(i)=0.0D0
          amnmed(i)=amntot(i)/nframes
!       write(*,*) i,amnmed(i),dp(i),amntot(i),desvio(nframes,i)
!!! removido v1.05r4             do j=1,nframes
!!! removido v1.05r4             dp(i)= dp(i) + (desvio(j,i)-amnmed(i))**2 
!!! removido v1.05r4             enddo
!!! removido v1.05r4         dp(i)= sqrt( dp(i) / nframes )
        enddo

        do i=1,namn
           write(34,*) res(i), i,amnmed(i) !!! removido v1.05r4 ,dp(i)
        enddo
        close(34)
